...
import { Wbase1070Form } from './Wbase1070Form';
import { Wbase1070Print } from './wbase1070Print';
import {
  FileText,
  Search,
  AlertTriangle,
...
interface Wbase1070PageProps {
  onBackToMenu?: () => void;
}

export const Wbase1070Page: React.FC<Wbase1070PageProps> = ({ onBackToMenu }) => {
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-slate-500'}`}>
              設定與管理標準基本備註 (PostgreSQL Table: 基本備註)
            </p>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xs p-4">
          ...
          >
            <div className="flex items-center space-x-3 text-red-500 mb-4">
              ...
              <h3 className="text-lg font-bold">確認刪除該筆基本備註？</h3>
            </div>
            <p className="text-sm mb-6 leading-relaxed">
              您即將刪除基本備註：
              <span className="font-bold text-yellow-400 ml-1">
                [{selectedRow.noteCode}] {selectedRow.noteName}
              </span>
              <br />
