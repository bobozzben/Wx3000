...
import { useWbase1070 } from './useWbase1070';
import { Printer, X, FileText } from 'lucide-react';
import type { NoteItemPrintFilter } from '../../services/wbase1070';
import { useTheme } from '../menu/ThemeContext';

export const Wbase1070Print: React.FC = () => {
  const { isDark } = useTheme();
  const { isPrintOpen, closePrint, fetchPrintDataList, printData, triggerReport, loading } =
    useWbase1070();

  const [filter, setFilter] = useState<NoteItemPrintFilter>({
    codeStart: '',
    codeEnd: '',
  });
          ...
          {!hasSearched ? (
            ...
            <div className="bg-white text-black p-6 rounded shadow max-w-3xl mx-auto print:max-w-none print:shadow-none">
              <div className="text-center mb-6 pb-2 border-b-2 border-black">
                <h1 className="text-2xl font-black tracking-widest">基本備註清冊名冊</h1>
              ...
              <table className="w-full text-left text-xs border-collapse border border-black">
                <thead>
                  <tr className="border-b-2 border-black bg-gray-100">
                    <th className="border border-black p-2 font-bold w-16 text-center">序次</th>
                    <th className="border border-black p-2 font-bold w-24">備註代號</th>
                    <th className="border border-black p-2 font-bold w-36">備註說明</th>
                    <th className="border border-black p-2 font-bold">備註內容</th>
                  </tr>
                </thead>
                <tbody>
                  {printData.map((row, idx) => (
                    <tr key={row.noteCode} className={idx % 2 === 1 ? 'bg-gray-50 border-b border-gray-300' : 'bg-white border-b border-gray-300'}>
                      <td className="border border-black p-2 text-center">{idx + 1}</td>
                      <td className="border border-black p-2 font-bold">{row.noteCode}</td>
                      <td className="border border-black p-2 font-semibold">{row.noteName}</td>
                      <td className="border border-black p-2 whitespace-pre-wrap">{row.content}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
