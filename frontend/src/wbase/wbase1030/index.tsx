...
interface Wbase1030PageProps {
  onBackToMenu?: () => void;
}

export const Wbase1030Page: React.FC<Wbase1030PageProps> = ({ onBackToMenu }) => {
  const {
    list,
    selectedItem,
    searchKeyword,
  ...
  const handleRowsChange = (newRows: EmpItem[]) => {
    useWbase1030.setState({ list: newRows });
  };

  const handleSaveRow = async (row: EmpItem) => {
    if (!row.empCode || !row.empCode.trim()) return;
    try {
    ...
    <div className="flex flex-col h-full min-h-screen bg-gray-100 p-4 font-mono">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-wide text-yellow-300">
              建檔人員資料維護作業 <span className="text-xs text-gray-300">[wbase1030]</span>
            </h1>
            <p className="text-xs text-gray-300">
              提供建檔人員 (員工) 主檔資料建置、編輯、刪除與清冊列印 (FoxPro 盲打 15 列版)。
            </p>
          </div>
        </div>
      ...
      <div className="flex-1 mb-3">
        <Wbase1030Form
          ...
          statusBarInfo={
            selectedItem
              ? `已選取: [${selectedItem.empCode}] ${selectedItem.empName}`
      ...
      {isDeleteConfirmOpen && selectedItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xs p-4 font-mono">
          <div className="bg-slate-900 border-4 border-red-600 rounded-lg p-6 max-w-md w-full text-white shadow-2xl text-center">
            ...
            <h3 className="text-xl font-bold text-red-400 mb-2">確認刪除人員資料？</h3>
            <p className="text-sm text-gray-300 mb-6">
              您即將刪除編號為{' '}
              <span className="text-yellow-400 font-bold">
                [{selectedItem.empCode}] {selectedItem.empName}
              </span>{' '}
              的資料，此動作無法復原！
            </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 font-mono">
          <div className="bg-blue-950 border-4 border-yellow-400 text-white p-6 rounded-lg shadow-2xl max-w-lg text-center animate-fade-in">
            ...
            <p className="text-base font-semibold mb-4 text-gray-200">
              目前共有 <span className="text-yellow-400 text-xl font-mono px-1">{list.length}</span> 筆人員記錄。
            </p>
