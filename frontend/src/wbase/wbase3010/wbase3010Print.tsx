...
import type { TicketPlacePrintFilter } from '../../services/wbase3010';
import { useTheme } from '../menu/ThemeContext';

export const Wbase3010Print: React.FC = () => {
  const { isDark } = useTheme();
  const { isPrintOpen, closePrint, fetchPrintDataList, printData, triggerReport, loading } =
    useWbase3010();

  const [filter, setFilter] = useState<TicketPlacePrintFilter>({
    codeStart: '',
    codeEnd: '',
  });
          ...
          {!hasSearched ? (
            ...
            <div className="bg-white text-black p-6 rounded shadow max-w-4xl mx-auto print:max-w-none print:shadow-none">
              <div className="text-center mb-6 pb-2 border-b-2 border-black">
                <h1 className="text-2xl font-black tracking-widest">統一發票購買地點清冊總表</h1>
              ...
              <table className="w-full text-left text-xs border-collapse border border-black">
                <thead>
                  <tr className="border-b-2 border-black bg-gray-100">
                    <th className="border border-black p-2 font-bold w-12 text-center">序次</th>
                    <th className="border border-black p-2 font-bold w-24">編號</th>
                    <th className="border border-black p-2 font-bold w-40">購買地點</th>
                    <th className="border border-black p-2 font-bold w-28">連絡人</th>
                    <th className="border border-black p-2 font-bold w-32">連絡電話</th>
                    <th className="border border-black p-2 font-bold">購買地址</th>
                  </tr>
                </thead>
                <tbody>
                  {printData.map((row, idx) => (
                    <tr
                      key={row.placeCode}
                      className={idx % 2 === 1 ? 'bg-gray-50 border-b border-gray-300' : 'bg-white border-b border-gray-300'}
                    >
                      <td className="border border-black p-2 text-center">{idx + 1}</td>
                      <td className="border border-black p-2 font-bold">{row.placeCode}</td>
                      <td className="border border-black p-2 font-semibold">{row.placeName}</td>
                      <td className="border border-black p-2">{row.contactPerson}</td>
                      <td className="border border-black p-2">{row.contactTel}</td>
                      <td className="border border-black p-2 whitespace-pre-wrap">{row.placeAddress}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
