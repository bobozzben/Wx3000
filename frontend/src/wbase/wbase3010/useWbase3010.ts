...
import type {
  TicketPlaceItem,
  TicketPlacePrintFilter,
  Waccrep3106bParams,
} from '../../services/wbase3010';
import {
  getTicketPlaceList,
  saveTicketPlace,
  batchSaveTicketPlaces,
  deleteTicketPlace,
  printTicketPlaceList,
  callWaccrep3106b,
} from '../../services/wbase3010';

interface Wbase3010State {
  rows: TicketPlaceItem[];
  setRows: (rows: TicketPlaceItem[]) => void;
  ...
  isPrintOpen: boolean;
  printData: TicketPlaceItem[];
  printFilter: TicketPlacePrintFilter;

  // Actions
  refreshData: (keyword?: string) => Promise<void>;
  handleSaveRow: (updatedRow: TicketPlaceItem) => Promise<void>;
  handleSaveAll: () => Promise<void>;
  openDeleteConfirm: (idx?: number) => void;
  confirmDelete: () => Promise<boolean>;
  openPrint: () => void;
  closePrint: () => void;
  fetchPrintDataList: (filter: TicketPlacePrintFilter) => Promise<TicketPlaceItem[]>;
  triggerReport: (params?: Waccrep3106bParams) => Promise<boolean>;
}

export const useWbase3010 = create<Wbase3010State>((set, get) => ({
  rows: [],
  ...
  refreshData: async (keyword) => {
    set({ loading: true });
    try {
      const data = await getTicketPlaceList(keyword);
      set({ rows: data, loading: false });
    } catch (e) {
      console.error('Failed to load ticket places:', e);
      set({ errorToast: '載入購票地點資料失敗，請重新連線', loading: false });
    }
  },

  handleSaveRow: async (updatedRow) => {
    if (!updatedRow.placeCode.trim()) return;
    try {
      const isNew = !get().rows.some((r) => r.placeCode === updatedRow.placeCode);
      const success = await saveTicketPlace(updatedRow, isNew);
      if (success) {
  ...
  handleSaveAll: async () => {
    set({ loading: true });
    try {
      const success = await batchSaveTicketPlaces(get().rows);
      if (success) {
        set({ showAutoCloseToast: true });
        setTimeout(() => set({ showAutoCloseToast: false }), 2000);
        await get().refreshData();
  ...
  confirmDelete: async () => {
    ...
    try {
      const res = await deleteTicketPlace(target.placeCode);
      if (res) {
        set({
      ...
      console.error('Delete ticket place error:', e);
      set({ errorToast: '刪除購票地點資料失敗' });
    } finally {
  ...
  fetchPrintDataList: async (filter) => {
    set({ loading: true });
    try {
      const data = await printTicketPlaceList(filter);
